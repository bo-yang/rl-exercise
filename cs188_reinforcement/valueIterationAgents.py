...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(0, self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                if len(possibleActions) < 1:
                    continue
                values[state] = max([self.getQValue(state, action) for action in possibleActions])
            self.values = values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState,prob in states_and_probs:
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            ...
            if len(possibleActions) < 1:
                continue
            # create predecessor mappings
            for action in possibleActions:
                states_and_probs = self.mdp.getTransitionStatesAndProbs(state, action)
                for nextState,prob in states_and_probs:
                    if prob > 0:
                        if nextState not in preds.keys():
                            preds[nextState] = set()
        ...
        for i in range(0, self.iterations):
            ...
            for p in preds[state]:
                ...
                qvalues = [self.getQValue(p, action) for action in possibleActions]
