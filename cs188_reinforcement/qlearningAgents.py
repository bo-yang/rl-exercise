...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "*** YOUR CODE HERE ***"
        self.qvalues = util.Counter() # (state, action) => q-value
    ...
    def update(self, state, action, nextState, reward):
        ...
        target = reward + self.discount * maxq
        self.qvalues[(state, action)] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * target
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            #    print 'weight[%s]=%f' %(key,self.weights[key])
